...
PROGRAM EXAMPLE
  ...
  BLOCK
    ...
    CALL REMOVE_WINDOW_MEAN(LENGTH, TOTAL, WINDOW, SIGNAL, SEQUENCE)
    SEQUENCE(1_IK::2_IK) = SEQUENCE(1_IK::2_IK)*WINDOW
    SEQUENCE(2_IK::2_IK) = SEQUENCE(2_IK::2_IK)*WINDOW
    WRITE(*, *) "FILTER(-)"
    PEAK = +1_IK
    RESULT = FREQUENCY_(FLAG, PEAK, METHOD, LENGTH, SEQUENCE)
    CALL AMPLITUDE_(FLAG, LENGTH, TOTAL, WINDOW, SIGNAL, RESULT, COS_AMP, SIN_AMP, AMP)
    WRITE(*, *) "1ST"
    WRITE(*, *) "FREQUENCY", RESULT, "ERROR", ABS(RESULT-FREQUENCY), "AMPLITUDE", COS_AMP, SIN_AMP
    PEAK = +2_IK
    RESULT = FREQUENCY_(FLAG, PEAK, METHOD, LENGTH, SEQUENCE)
    CALL AMPLITUDE_(FLAG, LENGTH, TOTAL, WINDOW, SIGNAL, RESULT, COS_AMP, SIN_AMP, AMP)
    WRITE(*, *) "2ND"
    WRITE(*, *) "FREQUENCY", RESULT, "ERROR", ABS(RESULT-2.0_RK*FREQUENCY), "AMPLITUDE", COS_AMP, SIN_AMP
    WRITE(*, *)
  END BLOCK
