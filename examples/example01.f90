...
PROGRAM EXAMPLE
  ...
  RESULT = FREQUENCY_(FLAG, PEAK, METHOD, LENGTH, SIGNAL)
  WRITE(*, *) "PARABOLA(FIT)  : ", RESULT, ABS(RESULT-FREQUENCY)
  WRITE(*, *)
  ...
  ! REFINE INITIAL GUESS (DIRECT)
  WRITE(*, *) "REFINED(DIRECT): "
  ...
  ! REFINE INITIAL GUESS (COMMON INTERFACE)
  WRITE(*, *) "REFINED(COMMON): "
  BLOCK
    REAL(RK) :: GUESS
    REAL(RK) :: INTERVAL
    INTEGER(IK) :: LIMIT
    ...
    LIMIT = 100_IK
    TOLERANCE = 1.0E-16_RK
    METHOD = REFINED_FFRFT
    RESULT = FREQUENCY_REFINED_(FLAG, METHOD, LENGTH, TOTAL, WINDOW, SIGNAL, GUESS, INTERVAL, LIMIT, TOLERANCE)
    WRITE(*, *) "FFRFT          : ", RESULT, ABS(RESULT-FREQUENCY)
    METHOD = REFINED_PARABOLA
    RESULT = FREQUENCY_REFINED_(FLAG, METHOD, LENGTH, TOTAL, WINDOW, SIGNAL, GUESS, INTERVAL, LIMIT, TOLERANCE)
    WRITE(*, *) "PARABOLA       : ", RESULT, ABS(RESULT-FREQUENCY)
    METHOD = REFINED_PARABOLA_FIT
    RESULT = FREQUENCY_REFINED_(FLAG, METHOD, LENGTH, TOTAL, WINDOW, SIGNAL, GUESS, INTERVAL, LIMIT, TOLERANCE)
    WRITE(*, *) "PARABOLA(FIT)  : ", RESULT, ABS(RESULT-FREQUENCY)
    METHOD = REFINED_SEARCH
    RESULT = FREQUENCY_REFINED_(FLAG, METHOD, LENGTH, TOTAL, WINDOW, SIGNAL, GUESS, INTERVAL, LIMIT, TOLERANCE)
    WRITE(*, *) "SEARCH         : ", RESULT, ABS(RESULT-FREQUENCY)
  END BLOCK
