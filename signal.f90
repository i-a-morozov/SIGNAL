...
MODULE SIGNAL
  ...
  SUBROUTINE FILTER_(LENGTH, SEQUENCE, LIMIT, SVD_LIST) &
    ...
    REAL(RK), DIMENSION(LENGTH/2_IK+1_IK, LENGTH/2_IK) :: COPY
    INTEGER(IK) :: I
    INTEGER :: NR, NC
    ...
    NR = INT(LENGTH)/2 + 1
    NC = INT(LENGTH)/2
    CALL DGEMM('N','T',NR,NC,NC,1.0_8,MATRIX,SIZE(MATRIX,1),V_MATRIX,SIZE(V_MATRIX,1),0.0_8,COPY,SIZE(COPY,1))
    MATRIX = 0.0_8
    NR = INT(LENGTH)/2 + 1
    NC = INT(LENGTH)/2
    CALL DGEMM('N','N',NR,NC,NC,1.0_8,U_MATRIX,SIZE(U_MATRIX,1),COPY,SIZE(COPY,1),0.0_8,MATRIX,SIZE(MATRIX,1))
