...
! STATIC LIB
! gfortran -c -cpp -fPIC -frecursive -std=f2018 -Wall -pedantic -O3 -ffast-math -march=native -Wno-unused-function signal.f90
...
#define __SEQUENCE__ SEQUENCE_SUM_
...
MODULE SIGNAL
  ...
  REAL(RK),    PUBLIC, PARAMETER :: SVD_LEVEL              = 1.0E-9_RK           ! SINGULAR VALUE THRESHOLD LEVEL
  ...
  INTEGER,     PUBLIC, PARAMETER :: SVD_BASIS              = 128_IK              ! MAX NUMBER OF BASIS VECTORS IN THE IMPLICITLY RESTARTED ARNOLDI PROCESS, OPTIMAL VALUE (?)
  INTEGER,     PUBLIC, PARAMETER :: SVD_LENGH              = 64_IK               ! LENGTH OF ARNOLDI FACTORIZATION
  ...
  PUBLIC :: WINDOW_           ! (SUB) (T) WINDOW_(<LENGTH>,<ORDER>,<SEQUENCE>) (COSINE WINDOW)
  ...
  PUBLIC :: SVD_TRUNCATED_    ! (SUB) (F) SVD_TRUNCATED_(<NR>,<NC>,<NS>,<MATRIX>(<NR>,<NC>),<LIST>(<NS>),<RVEC>(<NC>,<NR>),<LVEC>(<NR>,<NC>))
  ...
  SUBROUTINE LEAST_SQUARES_(NR, NC, MATRIX, VECTOR, SOLUTION)
    ...
    END DO
    V1 = VECTOR
    CALL DGEMV('T',NR,NR,1.0_8,U_MATRIX,NR,V1,1,0.0_8,V2,1)
    CALL DGEMV('T',NR,NC,1.0_8,COPY,NR,V2,1,0.0_8,V3,1)
    CALL DGEMV('N',NC,NC,1.0_8,V_MATRIX,NC,V3,1,0.0_8,V4,1)
    SOLUTION = REAL(V4, RK)
  ...
  ! SEQUENCE (SUM)
  SUBROUTINE SEQUENCE_SUM_(LENGTH, SEQUENCE, MATRIX)
    INTEGER(IK), INTENT(IN) :: LENGTH
    REAL(RK), DIMENSION(LENGTH), INTENT(OUT) :: SEQUENCE
    REAL(RK), DIMENSION(LENGTH/2_IK+1_IK, LENGTH/2_IK), INTENT(IN) :: MATRIX
    INTEGER(IK) :: ROW
  ...
  SUBROUTINE FILTER_(LENGTH, SEQUENCE, LIMIT, SVD_LIST) &
    ...
    NC = INT(LENGTH)/2_IK
    NS = LIMIT
    CALL DGEMM('N','T',NS,NC,NS,1.0_8,DIAG,SIZE(DIAG,1),V_MATRIX,SIZE(V_MATRIX,1),0.0_8,COPY,SIZE(COPY,1))
    CALL DGEMM('N','N',NR,NC,NS,1.0_8,U_MATRIX,SIZE(U_MATRIX,1),COPY,SIZE(COPY,1),0.0_8,MATRIX,SIZE(MATRIX,1))
...
! 23_09_2020
! TYPE WRAPPERS AROUND DGESVD, DGEMV AND DGEMM ARE REMOVED (TESTED)
! ADDED TRUNCATED SVD (ARPACK), LEFT VECTOR ARE COMPUTED DIRECTRLY