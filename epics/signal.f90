...
MODULE SIGNAL
  ...
  SUBROUTINE SVD_(NR, NC, MATRIX, SVD_LIST, U_MATRIX, V_MATRIX)
    ...
    INTEGER :: WORK_SIZE
    INTEGER :: INFO
    REAL(8), DIMENSION(NR, NC) :: D_MATRIX
    REAL(8), DIMENSION(MIN(NR, NC)) :: D_SVD_LIST
    REAL(8), DIMENSION(NR, NR) :: D_U_MATRIX
    REAL(8), DIMENSION(NC, NC) :: D_V_MATRIX
    D_MATRIX = REAL(MATRIX, 8)
    WORK_SIZE = SIZE(WORK)
    CALL DGESVD('A','A',INT(NR),INT(NC),D_MATRIX,INT(NR),D_SVD_LIST,D_U_MATRIX,INT(NR),D_V_MATRIX,INT(NC),WORK,WORK_SIZE,INFO)
  ...
  SUBROUTINE FILTER_(LENGTH, SEQUENCE, LIMIT, SVD_LIST) &
    ...
    REAL(8), DIMENSION(LENGTH/2_IK+1_IK, LENGTH/2_IK) :: D_MATRIX
    REAL(8), DIMENSION(LENGTH/2_IK+1_IK, LENGTH/2_IK+1_IK) :: D_U_MATRIX
    REAL(8), DIMENSION(LENGTH/2_IK, LENGTH/2_IK) :: D_V_MATRIX
    REAL(8), DIMENSION(LENGTH/2_IK+1_IK, LENGTH/2_IK) :: COPY
    INTEGER(IK) :: I
    INTEGER :: NR, NC
    ...
    NR = INT(LENGTH)/2 + 1
    NC = INT(LENGTH)/2
    CALL DGEMM('N','T',NR,NC,NC,1.0_8,D_MATRIX,SIZE(D_MATRIX,1),D_V_MATRIX,SIZE(D_V_MATRIX,1),0.0_8,COPY,SIZE(COPY,1))
    D_MATRIX = 0.0_8
    NR = INT(LENGTH)/2 + 1
    NC = INT(LENGTH)/2
    CALL DGEMM('N','N',NR,NC,NC,1.0_8,U_MATRIX,SIZE(U_MATRIX,1),COPY,SIZE(COPY,1),0.0_8,D_MATRIX,SIZE(D_MATRIX,1))
    MATRIX = REAL(D_MATRIX, RK)
