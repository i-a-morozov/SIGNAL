...
MODULE FREQUENCY
    ...
    INTEGER,PARAMETER  :: FLA=2                          ! COMPLEX SIGNAL FLAG (FLA=2 FOR REAL INPUT SIGNAL AND FLA=1 FOR COMPLEX INPUT SIGNAL)
CONTAINS
...
2       IF (N.GT.LIM) THEN
    ...
    PURE SUBROUTINE WINDOW(ORD,ARR) 
        ...
        END DO
        !WIN = MUL*(1._RK+COS(TWO_PI*([(REAL(I-1,RK),I=1,NUM,1)]/REAL(NUM,RK)-0.5_RK)))**ORD
        ARR=ARR*WIN
    ...
    PURE REAL(RK) FUNCTION SEARCH(ORD,ARR)
        ...
        LOC=(ARR-SUM(WIN*ARR)/SUM(WIN))*WIN
        ! FFT
        FOU=LOC
        CALL FFT(NUM,1,FOU)
        ...
        END DO
        ! LOC = LOC*EXP(CMPLX([(0.0_RK,I=1,NUM,1)],FAC*[(REAL(I-1,RK),I=1,NUM,1)],RK))
        ! FFRFT
